...
Ported from v1's agent/gmail_poller.py. Uses a Google Workspace service
account with domain-wide delegation to impersonate the shared inbox users.

Django-agnostic: no Django ORM imports. Returns EmailMessage DTOs.
"""
...
class GmailPoller:
    ...
    def _ensure_label(self, service, inbox_email: str) -> Optional[str]:
        """Ensure the processed label exists; return its ID."""
        try:
    ...
    def _parse_message(self, service, msg_data: dict, inbox_email: str) -> Optional[EmailMessage]:
        """Parse a Gmail API message response into an EmailMessage DTO."""
        try:
            headers = {
    ...
    def mark_processed(self, email_msg: EmailMessage):
        """Apply the processed label to an email AFTER successful DB persist.

        Called by pipeline to prevent email loss -- label is only applied
        once the email is safely persisted to the database.
        """
